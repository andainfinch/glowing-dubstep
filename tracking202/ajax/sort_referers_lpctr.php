...
//only create a new list if there is no post
	if (($_POST['order']== '') and ($_POST['offset'] == '')) {
		...
		//delete old referer list
			$mysql['user_id'] = mysql_real_escape_string($_SESSION['user_id']);
			$referer_sql = "DELETE FROM `202_sort_referers_lpctr` WHERE `user_id`='".$mysql['user_id']."'";
		...
		//run query
			while ($info_row = mysql_fetch_assoc($info_result)) {
				...
				// Clean escape characters from strings
					$mysql['sort_referer_click_throughs'] = mysql_real_escape_string($total_click_throughs);
					$mysql['sort_referer_ctr'] = mysql_real_escape_string($total_ctr_ratio);
				...
				//insert the data
					$referer_sort_sql = "INSERT INTO 202_sort_referers_lpctr
										 SET         user_id='".$mysql['user_id']."',
													 referer_id='".$mysql['site_domain_id']."',
													 sort_referer_clicks='".$mysql['sort_referer_clicks']."',

													 sort_referer_click_throughs='".$mysql['sort_referer_click_throughs']."',
 													 sort_referer_ctr='".$mysql['sort_referer_ctr']."',
...
$html['sort_referer_click_throughs_order'] = 'sort_referer_click_throughs asc';
if ($_POST['order'] == 'sort_referer_click_throughs asc') {
	$html['sort_referer_click_throughs_order'] = 'sort_referer_click_throughs desc';
	$mysql['order'] = 'ORDER BY `sort_referer_click_throughs` DESC';
} elseif ($_POST['order'] == 'sort_referer_click_throughs desc') {
	$html['sort_referer_click_throughs_order'] = 'sort_referer_click_throughs asc';
	$mysql['order'] = 'ORDER BY `sort_referer_click_throughs` ASC';
}

$html['sort_referer_ctr_order'] = 'sort_referer_ctr asc';
if ($_POST['order'] == 'sort_referer_ctr asc') {
	$html['sort_referer_ctr_order'] = 'sort_referer_ctr desc';
	$mysql['order'] = 'ORDER BY `sort_referer_ctr` DESC';
} elseif ($_POST['order'] == 'sort_referer_ctr desc') {
	$html['sort_referer_ctr_order'] = 'sort_referer_ctr asc';
	$mysql['order'] = 'ORDER BY `sort_referer_ctr` ASC';
}
...
<table cellspacing="0" cellpadding="0" style="width: 100%; font-size: 12px;">
	<tr>
		<td width="100%;">
			<a target="_new" href="/tracking202/analyze/referers_lpctr_download.php">
...
<table cellpadding="0" cellspacing="1" class="m-stats">
	<tr>
		<th><a class="onclick_color" onclick="loadContent('/tracking202/ajax/sort_referers_lpctr.php','','<? echo $html['sort_referer_referer_order']; ?>');">Referer</a></th>
		<th><a class="onclick_color" onclick="loadContent('/tracking202/ajax/sort_referers_lpctr.php','','<? echo $html['sort_referer_clicks_order']; ?>');">Clicks</a></th>
		<th><a class="onclick_color" onclick="loadContent('/tracking202/ajax/sort_referers_lpctr.php','','<? echo $html['sort_referer_click_throughs_order']; ?>');">Click Throughs</a></th>
		<th><a class="onclick_color" onclick="loadContent('/tracking202/ajax/sort_referers_lpctr.php','','<? echo $html['sort_referer_ctr_order']; ?>');">LP CTR</a></th>
		<th><a class="onclick_color" onclick="loadContent('/tracking202/ajax/sort_referers_lpctr.php','','<? echo $html['sort_referer_leads_order']; ?>');">Leads</a></th>
		<th><a class="onclick_color" onclick="loadContent('/tracking202/ajax/sort_referers_lpctr.php','','<? echo $html['sort_referer_su_ratio_order']; ?>');">S/U</a></th>
		<th><a class="onclick_color" onclick="loadContent('/tracking202/ajax/sort_referers_lpctr.php','','<? echo $html['sort_referer_payout_order']; ?>');">Payout</a></th>
		<th><a class="onclick_color" onclick="loadContent('/tracking202/ajax/sort_referers_lpctr.php','','<? echo $html['sort_referer_epc_order']; ?>');">EPC</a></th>
		<th><a class="onclick_color" onclick="loadContent('/tracking202/ajax/sort_referers_lpctr.php','','<? echo $html['sort_referer_avg_cpc_order']; ?>');">Avg CPC</a></th>
		<th><a class="onclick_color" onclick="loadContent('/tracking202/ajax/sort_referers_lpctr.php','','<? echo $html['sort_referer_income_order']; ?>');">Income</a></th>
		<th><a class="onclick_color" onclick="loadContent('/tracking202/ajax/sort_referers_lpctr.php','','<? echo $html['sort_referer_cost_order']; ?>');">Cost</a></th>
		<th><a class="onclick_color" onclick="loadContent('/tracking202/ajax/sort_referers_lpctr.php','','<? echo $html['sort_referer_net_order']; ?>');">Net</a></th>
		<th><a class="onclick_color" onclick="loadContent('/tracking202/ajax/sort_referers_lpctr.php','','<? echo $html['sort_referer_roi_order']; ?>');">ROI</a></th>
	</tr>
...
while ($referer_row = mysql_fetch_array($referer_result, MYSQL_ASSOC)) {
	...
	if (!$referer_row['site_domain_host']) {
		...
		$html['referer'] = '<a target="_new" title="'.$referer_row['site_domain_host'].'" href="http://'.$referer_row['site_domain_host'].'">'.$html['referer'].'</a>';
	}

	error_reporting(0);

	$html['sort_referer_clicks'] = htmlentities($referer_row['sort_referer_clicks'], ENT_QUOTES, 'UTF-8');
	$html['sort_referer_click_throughs'] = htmlentities($referer_row['sort_referer_click_throughs'], ENT_QUOTES, 'UTF-8');
	$html['sort_referer_ctr'] = htmlentities($referer_row['sort_referer_ctr'], ENT_QUOTES, 'UTF-8');
	...
	<tr>
		...
		<td class="m-row1"><? echo $html['sort_referer_click_throughs']; ?></td>
		<td class="m-row1"><? echo $html['sort_referer_ctr']; ?></td>
...
<? if ($query['pages'] > 2) { ?>
	<div class="offset">   <?
		if ($query['offset'] > 0) {
			printf(' <a class="onclick_color" onclick="loadContent(\'sort_referers.php\',\'%s\',\'%s\');">First</a> ', $i, $html['order']);
			printf(' <a class="onclick_color" onclick="loadContent(\'sort_referers.php\',\'%s\',\'%s\');">Prev</a> ', $query['offset'] - 1, $html['order']);
		}
		...
		if ($query['pages'] > 1) {
			for ($i=0; $i < $query['pages']-1; $i++) {
				if (($i >= $query['offset'] - 10) and ($i < $query['offset'] + 11)) {
					...
					printf(' <a %s onclick="loadContent(\'sort_referers.php\',\'%s\',\'%s\');">%s</a> ', $class, $i, $html['order'], $i+1);
					unset($class);
				}
			}
		}
		...
		if ($query['offset'] < $query['pages'] - 2) {
			printf(' <a class="onclick_color" onclick="loadContent(\'sort_referers.php\',\'%s\',\'%s\');"">Next</a> ', $query['offset'] + 1, $html['order']);
			printf(' <a class="onclick_color" onclick="loadContent(\'sort_referers.php\',\'%s\',\'%s\');">Last</a> ', $query['pages'] - 2, $html['order']);
		} ?>
	</div>
<? } ?>
